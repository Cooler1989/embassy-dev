...
impl OpenThermMessage {
    fn GetU32Value(&self) -> u32 {
        let val: u32 = self.data_id_ as u32 | <OpenThermMessageValue as Into<u32>>::into(self.value_);
        val
    }
}

impl From<OpenThermMessageValue> for u32 {
    fn from(item: OpenThermMessageValue) -> Self {
        match item {
            OpenThermMessageValue::Value(v) => v as u32,
        }
    }
}
...
impl From<OpenThermMessageValue> for u16 {
    fn from(item: OpenThermMessageValue) -> Self {
        match item {
            OpenThermMessageValue::Value(v) => v as u16,
            _ => 0u16,
        }
        //  item.Value as u16
    }
...
#[trait_variant::make(MultiErrorType: Send)]
pub trait ErrorType {
    /// Error type
    type Error: Error;
}
...
#[trait_variant::make(MultiOpenThermDevice: Send)]
...
#[trait_variant::make(MultiOpenThermBus: Send)]
